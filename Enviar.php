...
if ($resultado3->num_rows > 0) {
    ...
    while ($fila = $resultado3->fetch_assoc()) {
        ...
        $consulta4 = "SELECT fecha FROM usuario WHERE idUsuario = $idUsuario";
        $resultadoFecha = mysqli_query($conexion, $consulta4);

        // Verificar si se encontró la fecha en la base de datos
        if ($resultadoFecha->num_rows > 0) {
            // Obtener la fecha de la base de datos
            $filaFecha = $resultadoFecha->fetch_assoc();
            $fecha_db = $filaFecha["fecha"];

            // Comparar la fecha de la base de datos con una hora establecida (ejemplo: 12:00 PM)
            $hora_establecida = strtotime("01:48:00");
            $hora_db = strtotime($fecha_db);

            if ($hora_db >= $hora_establecida) {
